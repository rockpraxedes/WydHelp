...
function loadForDate( profileId: string, dateKey: string ): TodayState {
  try {
    ...
    if ( raw ) return { guildActive: false, ...JSON.parse( raw ) }
  } catch { }
  return { checked: {}, eventActive: false, guildActive: false }
}
...
export function useDailyMissions( profileId: string, dateKey: string ) {
    ...
    if ( !hasStoredState ) {
      ...
      if ( histEntry ) {
        const restored: TodayState = {
          ...
          guildActive: histEntry.guildActive ?? false,
        }
        setState( restored )
        return
      }
  ...
  const toggleGuild = useCallback( ( active: boolean ) => {
    setState( prev => {
      const next = { ...prev, guildActive: active }
      saveForDate( profileId, dateKey, next )
      return next
    } )
  }, [ profileId, dateKey ] )
  ...
  const saveDay = useCallback( ( missionNames: string[] ) => {
    ...
    setHistory( loadHistory( profileId ) )
    return true
  }, [ profileId, dateKey, state.eventActive, state.guildActive ] )
